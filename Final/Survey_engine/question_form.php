...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sur_id = $qstn = $desc = $ans1 = $ans2 = $ans3 = $ans4 = $ans5 = "";
    ...
    if (empty($errors)) {
        ...
        $qstn_id;   //To refrence the answers
        $question = new Question($qstn, $cat);
        
        //Submmiting Question
        $dbc = $question->getDB();
        $qstn_id = $question->insert_qstn($dbc);
        
        
          //ans1
            if(!empty($_POST['ans1'])) {
                $question->insert_ans(test_input($_POST['ans1']), $dbc);
                $question->insert_into_survey($sur_id, $qstn_id, test_input($_POST['ans1']), $dbc);
           }

            //ans2
            if(!empty($_POST['ans2'])) {
                $question->insert_ans(test_input($_POST['ans2']), $dbc);
                $question->insert_into_survey($sur_id, $qstn_id, test_input($_POST['ans2']), $dbc);
            }

            //ans3
            if(!empty($_POST['ans3'])) {
                $question->insert_ans(test_input($_POST['ans3']), $dbc);
                $question->insert_into_survey($sur_id, $qstn_id, test_input($_POST['ans3']), $dbc);
            }

            //ans4
            if(!empty($_POST['ans4'])) {
                $question->insert_ans(test_input($_POST['ans4']), $dbc);
                $question->insert_into_survey($sur_id, $qstn_id, test_input($_POST['ans4']), $dbc);
            }

            //ans5
            if(!empty($_POST['ans5'])) {
                $question->insert_ans(test_input($_POST['ans5']), $dbc);
                $question->insert_into_survey($sur_id, $qstn_id, test_input($_POST['ans5']), $dbc);
            }
        mysqli_close($dbc);
    }      
}
...
<body>
    <form action="question_form.php" method="post" >
        <p><b>Select Target Survey:</b></br> <select name="survey"><option>Select A Survey</option>
            <?php 
            $slave_qstn = new Question("","");
            $dbc = $slave_qstn->getDB();
			...
			echo ">$row[1]</option>\n";
                        ...
                        echo '<option>Please add a new survey first.</option>';
                }
